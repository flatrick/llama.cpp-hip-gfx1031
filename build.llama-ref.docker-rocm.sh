...
REF="latest"
IMAGE_PREFIX="llama-cpp-gfx1031"
IMAGE_TAG=""
FORCE=0
KEEP_WORKTREE=0
WORKDIR=""
...
usage() {
  cat <<'EOF'
Usage: build.llama-ref.docker-rocm.sh [--ref REF] [--image-tag TAG] [--force] [--keep-worktree]

Fetch an upstream llama.cpp ref into a temporary checkout and build it as a
separate ROCm image without modifying the pinned local submodule.
...
"$SCRIPT_DIR/build.docker-rocm.sh" "${BUILD_ARGS[@]}"
...
Run an existing launcher against it with:
  ROCM_IMAGE=$IMAGE_NAME bash Qwen3.5-4B-UD-Q5_K_XL.82k.sh

Or swap in another launcher the same way:
  ROCM_IMAGE=$IMAGE_NAME bash Qwen3.5-4B-UD-Q5_K_XL.sh
EOF
